...
public class MapProtocol extends Thread{
		...
		while(App.mapProtocolTerminationFlag == false){
			...
			if(App.isProcessActive == true){
				...
				for(int i=0; i<messageCount; i++){
					...
					Integer clockValue = App.vectorClock.get(App.self.getNodeId());
					App.vectorClock.set(App.self.getNodeId(), clockValue + 1);
					...
					try{
						...
						socket.close();
						System.out.println("Sending map message, src : " + m.getSrcNodeID() + " ,dst : " + neighborSelected);
					}catch(Exception e){
						//
					}
	...
	public static void sendNodePassiveMessage() {
